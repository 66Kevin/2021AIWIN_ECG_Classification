...
class SE_ECGNet(nn.Module):
    ...
    def __init__(self, struct=[(1, 3), (1, 5), (1, 7)], num_classes=2):
        super().__init__()
        self.struct = struct
        ...
        self.fc = nn.Linear(in_features=384 * len(struct), out_features=num_classes)
        ...
        for i, kernel_size in enumerate(self.struct):
            ...
            block3 = self._make_layer(in_channels=384, out_channels=384, kernel_size=kernel_size[1], stride=2,
...
#     input = torch.randn(16, 12, 5000)
#     # info = torch.randn(16, 2)
#     model = SE_ECGNet()
#     output = model(input)
#     # print(model)

#     # output: (batch_size, classnum)
#     print(output.shape)
